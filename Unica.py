...
for index in range(0,len(onlyfiles),1):
    ...
    norm_l = np.linalg.norm(values_l)
    values_l = (values_l/norm_l)*100
    values_l = np.log1p(values_l)

    
    h=1/100
    for i in range(0,values_l.shape[0],1):
        if i!=values_l.shape[0]-1:
            div[i] = (values_l[i+1]-values_l[i])/h
            #print(div[i])

    div = cv2.GaussianBlur(div,(21,21),10)
    div = cv2.GaussianBlur(div,(5,5),10)
    div_sub = np.array_split(div,2)
    div_first = div_sub[0]
    div_second = div_sub[1]

    min_value = np.min(div_second)
    result = np.where(div == min_value)
    index_minimo = result[0][0]
    print("min index horizontal: ", index_minimo)
        
    for j in range(0,segment.shape[0],1):
        segment[j][index_minimo]=255
        
    max_value = np.max(div_first)
    result = np.where(div == max_value)
    index_maximo = result[0][0]
    print("max index horizontal: ",index_maximo)
    ...
    values_w = cv2.GaussianBlur(values_w,(21,21),15)
    div = np.zeros((values_w.shape[0],1))
    norm_w = np.linalg.norm(values_w)
    values_w = (values_w/norm_w)*100
    values_w = np.log1p(values_w)
    

    h=1/100
    for i in range(0,values_w.shape[0],1):
        if i!=values_w.shape[0]-1:
            div[i] = (values_w[i+1]-values_w[i])/h
            #print(div[i])
    
    div = cv2.GaussianBlur(div,(21,21),10)
    div = cv2.GaussianBlur(div,(5,5),10)
    #print("Div_sub:", div_sub)
    div_sub = np.array_split(div,2)
    div_first = div_sub[0]
    div_second = div_sub[1]
    ...
    result = np.where(div == min_value)
    index_minimo = result[0][0]
    print("min index vertical: ", index_minimo)
    ...
    result = np.where(div == max_value)
    index_maximo = result[0][0]
    print("max index vertical: ",index_maximo)
