...
def print_quad(string,position):
    ...
    #reset the txt
    f.writelines("@"+onlyfiles[index]+"@"+str(position)+"@"+"\n")
    f.close()
    return index
def prox_file(var):
    f = open("text.txt","r")
...
while(1):
    ...
    if contador[1]==1:
        for i in [0,1,2,3]:
            if i==3:
                cv2.line(img,pos[i-3],pos[i],(0,255,0),10)
            else:
                cv2.line(img,pos[i],pos[i+1],(0,255,0),10)
        path = (mypath+"_mark/"+onlyfiles[index])
        print(path)
        cv2.imwrite(path,img)
        print_quad((onlyfiles[index]),pos)
        pos = [[0,0],[0,0],[0,0],[0,0]]
        contador[1] = 0
        contador[0] = 0
